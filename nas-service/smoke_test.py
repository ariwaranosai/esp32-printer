...
def check(url, config, output=None, layout="legacy"):
    size = (456, 656) if layout == "compact" else (432, 576)
    ...
    try:
        ...
        for index in range(2):
            response = session.get(base + '/photo.jpg', headers={'Authorization': 'Bearer ' + token, 'X-Photo-Layout': layout},
        ...
        print(f'PASS: health, authentication, two {size[0]}x{size[1]} RGB baseline JPEG responses')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--layout', choices=['legacy', 'compact'], default='legacy')
    args = parser.parse_args()
    try:
        check(args.url, args.config, args.output, args.layout)
