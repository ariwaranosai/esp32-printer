...
- `photo_refresh_seconds`：照片更换间隔，默认 10800 秒（3 小时），范围 3600–604800；在每次设备唤醒时检查是否到期。天气和传感器仍按 `refresh_seconds` 更新；若将设备唤醒间隔设置得更长，照片也只能在唤醒时更新。
...
- 按官方原理图，ALDO3 标为 `Audio_VCC`，但实机关闭后 RTC/SHTC3 的 I²C 通信异常，因此保留供电，仅将音频功放控制脚置低；ALDO4 为 `EPD_VCC`，每次唤醒恢复，刷新结束后关闭。屏幕信号脚在休眠时保持低电平，避免经 GPIO 向断电屏幕反向供电。电源寄存器修改保留其他电源使能位，并读回确认。
